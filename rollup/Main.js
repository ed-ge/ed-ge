    ...
    */
    ...
    const SceneManager = {
      ...
      set currentScene(argument) {
        ...
        this.scenes[this._currentSceneIndex].start2(Base$1.Globals.GameBehaviors, Base$1.Globals.GameObjects, Engine.Components);
      },
    ...
    function main(gameObjects, gameBehaviors, scenes) {
      Base$1.Globals.gameObjects = gameObjects;
      Base$1.Globals.components = Components;
      Base$1.Globals.gameBehaviors = gameBehaviors;
      let canv, ctx;

      scenes.allScenes
        .forEach(i => SceneManager.addScene(Base$1.Globals.parse(i)));
      ...
      function keydown(event) {
        if (Base$1.Input.keys[event.key] != true)
          Base$1.Input.down[event.key] = true;
        Base$1.Input.keys[event.key] = true;
      }

      function keyup(event) {
        if (Base$1.Input.keys[event.key] != false)
          Base$1.Input.up[event.key] = true;
        Base$1.Input.keys[event.key] = false;
      }

      function mousedown(event) {
        if (Base$1.Input.mouseButtons[event.button] != true)
          Base$1.Input.mouseButtonsDown[event.button] = true;
        Base$1.Input.mouseButtons[event.button] = true;
      }

      function mouseup(event) {
        if (Base$1.Input.mouseButtons[event.button] != false)
          Base$1.Input.mouseButtonsUp[event.button] = true;
        Base$1.Input.mouseButtons[event.button] = false;
      }

      function mousemove(event) {
        [Base$1.Input.mousePosition.x, Base$1.Input.mousePosition.y] = [event.clientX, event.clientY];

      }

      function wheelevent(event) {
        if (event.deltaY != 0)
          Base$1.Input.mouseScrollDelta = event.deltaY;
      }
